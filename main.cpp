...
void monitorujStan(vector<Plane>* samoloty, atomic<bool>* stop, atomic<bool>* kolizja) {
    while (!stop->load()) {
...
bool generujNowySamolot(vector<Plane>& samoloty, int& literaIndex) {
    ...
    cout << "Pojawil sie samolot " << litera << " na pozycji ("
        << (kierunek ? 0 : RAMKA_X) << ", " << 10-y
        << ") po stronie " << (kierunek ? "prawej" : "lewej")
        << " na wysokosci " << 10-y << endl;
    literaIndex++;
    return true;
}
...
bool dodajSamolotW6Turze(vector<Plane>& samoloty, int& literaIndex) {
    ...
    auto wynik = znajdzNajbezpieczniejszeY(samoloty, kierunek);
    int y = wynik.first;
    int bezpieczenstwo = wynik.second;
...
int main() {
    ...
    int tura = 0;
    atomic<bool> stop_monitoring(false);
    atomic<bool> wykryto_kolizje(false);
    ...
    thread monitor_thread([&]() {
        monitorujStan(&samoloty, &stop_monitoring, &wykryto_kolizje);
        });
